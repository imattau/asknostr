...
export const Header: React.FC<HeaderProps> = ({
  ...
  const btnHover = theme === 'light' ? 'hover:bg-slate-100' : 'hover:bg-white/5'
  const btnBorder = theme === 'light' ? 'border-slate-200' : 'border-white/5'
  ...
  return (
    <header className={`border-b ${bgClass} flex items-center justify-between px-4 shrink-0 z-[1001] backdrop-blur-xl gap-2 overflow-hidden transition-all duration-300 ease-in-out ${isHeaderHidden ? 'h-0 opacity-0 border-b-0' : 'h-14 opacity-100'}`}>
      <div className="flex items-center gap-3 min-w-0">
        <img src="/asknostr_logo.png" alt="" className={`w-7 h-7 rounded-full border ${theme === 'light' ? 'border-slate-200' : 'border-slate-800'} shadow-[0_0_15px_rgba(168,85,247,0.3)] shrink-0`} />
      ...
      <div className="flex gap-2 items-center uppercase font-bold text-[9px] font-mono shrink-0">
        ...
        <button onClick={() => setLayout(layout === 'swipe' ? 'classic' : 'swipe')} className={`flex items-center gap-1.5 ${btnHover} px-2 py-1 rounded border ${btnBorder} transition-all text-slate-400 hidden sm:flex`}><Layout size={14} /> {layout === 'swipe' ? 'Classic' : 'Mobile'}</button>
        <button onClick={() => setTheme(theme === 'terminal' ? 'modern' : theme === 'modern' ? 'light' : 'terminal')} className={`flex items-center gap-1.5 ${btnHover} px-2 py-1 rounded border ${btnBorder} transition-all text-slate-400 hidden sm:flex`}><TerminalIcon size={14} /> Theme: {theme}</button>
