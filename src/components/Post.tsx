...
const PostComponent: React.FC<PostProps> = ({
  ...
  const addOptimisticReaction = useStore((state: any) => state.addOptimisticReaction)
  const addOptimisticDeletion = useStore((state: any) => state.addOptimisticDeletion)
  const addOptimisticApproval = useStore((state: any) => state.addOptimisticApproval)
  const optimisticDeletions = useStore((state: any) => state.optimisticDeletions)
  
  const postOptimistic = useStore((state: any) => state.optimisticReactions[event.id])
  const isOptimisticallyApproved = useStore((state: any) => state.optimisticApprovals.includes(event.id))
  
  const { subscribedCommunities } = useSubscriptions()
  
  const layout = useUiStore((state: any) => state.layout)
  const stack = useUiStore((state: any) => state.stack)
  const pushLayer = useUiStore((state: any) => state.pushLayer)
  const openLightbox = useUiStore((state: any) => state.openLightbox)
